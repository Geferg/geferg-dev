...
type KeyAccent = "neutral" | "blue" | "red" | "green";
type SequenceKey = "g" | "e" | "f" | "r";
...
const SEQUENCE_KEYS: readonly SequenceKey[] = ["g", "e", "f", "r"];
...
export default function ZmkGraphic({ active }: ZmkGraphicProps) {
    ...
    const cancelAnimations = useCallback((animations: Animation[]) => {
    ...
    const getKeyElements = useCallback(
        (keyId: SequenceKey): KeyElements | null => {
            ...
            const cap =
                key?.querySelector<HTMLElement>(".zmk-key__cap") ?? null;
            ...
            const label =
                key?.querySelector<HTMLElement>(".zmk-key__label") ?? null;
    ...
    const playTypingSequence = useCallback(() => {
        ...
        for (const typingEvent of TYPING_SEQUENCE) {
            const elements = getKeyElements(typingEvent.keyId);
            ...
            const pressedAt = attackMs / typingEvent.holdMs;
            const releasedAt =
    ...
    }, [cancelAnimations, getKeyElements]);

    const stopTypingSequence = useCallback(() => {
        /*
         * Capture the rendered state before cancelling anything. Once the
         * source animations are cancelled, their animated values disappear.
         */
        const renderedStates = SEQUENCE_KEYS.flatMap((keyId) => {
            ...
            const capStyle = getComputedStyle(elements.cap);
            const surfaceStyle = getComputedStyle(
                elements.pressSurface,
            );
            const labelStyle = getComputedStyle(elements.label);
        ...
        /*
         * Cancelling here removes both active presses and presses whose
         * delay has not yet elapsed.
         */
        cancelAnimations(typingAnimationsRef.current);
        cancelAnimations(releaseAnimationsRef.current);

        const releases: Animation[] = [];
        ...
        for (const state of renderedStates) {
            ...
            const surfaceIsVisible = surfaceOpacity > 0.001;
        ...
        releaseAnimationsRef.current = releases;
    }, [cancelAnimations, getKeyElements]);
